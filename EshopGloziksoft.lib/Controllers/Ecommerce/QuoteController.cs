...
namespace eshopgloziksoft.lib.Controllers.Ecommerce
{
    ...
    public class QuoteController : _BaseController
    {
        ...
        {
            QuoteModel.RecalcQuoteDiscountForSession(this.CurrentSessionId, EshopgloziksoftCustomerCache.CurrentMemberId);
        ...
        public ActionResult BasketDeliveryData()
        {
            ...
            if ((model.CurrentQuote.User.IsNew || model.CurrentQuote.User.IsEmpty) && EshopgloziksoftCustomerCache.IsCustomerAuthenticated)
            {
                EshopgloziksoftCustomer customer = EshopgloziksoftCustomerCache.GetCurrentCustomer();
                if (customer != null)
                {
        ...
        public ActionResult SubmitBasketDeliveryData(BasketModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (tmpUser.IsInternationalInvoice || tmpUser.IsInternationalTransport)
                {
                    ...
                    if (bankTransfer != null && model.PaymentTypeKey != bankTransfer.pk)
                    {
            ...
            if (ModelState.IsValid)
            {
                if (EshopgloziksoftCustomerCache.IsCustomerAuthenticated)
                {
                    EshopgloziksoftCustomer customer = EshopgloziksoftCustomerCache.GetCurrentCustomer();
                    if (customer != null)
                    {
            ...
            if (ModelState.IsValid)
            {
                ...
                //{
                //    return this.RedirectToEshopgloziksoftUmbracoPage(ConfigurationUtil.Ecommerce_Basket_Gateway_DhlParcelShopPageId);
                //}
                ...
                //{
                //    return this.RedirectToEshopgloziksoftUmbracoPage(ConfigurationUtil.Ecommerce_Basket_Gateway_SpsParcelShopPageId);
                //}
                //else
                {
                    return this.RedirectToEshopgloziksoftUmbracoPage(ConfigurationUtil.Ecommerce_Basket_ReviewAndSendPageId);
                }
            }
        ...
        ActionResult BasketFinalize()
        {
            ...
            //if (isCardPayment)
            //{
            //    return this.RedirectToEshopgloziksoftUmbracoPage(ConfigurationUtil.Ecommerce_Basket_CardPayPageId, string.Format("id={0}", quoteModel.pk.ToString()));
            //}
            //if (isSporopayPayment)
            //{
            //    return this.RedirectToEshopgloziksoftUmbracoPage(ConfigurationUtil.Ecommerce_Basket_SporopayPageId, string.Format("id={0}", quoteModel.pk.ToString()));
            //}
            //if (isVubeplatbyPayment)
            //{
            //    return this.RedirectToEshopgloziksoftUmbracoPage(ConfigurationUtil.Ecommerce_Basket_VubeplatbyPageId, string.Format("id={0}", quoteModel.pk.ToString()));
            //}

            return this.RedirectToEshopgloziksoftUmbracoPage(ConfigurationUtil.Ecommerce_Basket_FinishedPageId);
        }
        ...
        public ActionResult QuoteSendStateNotification(string id)
        {
            ...
            model.CurrentQuote = GetQuoteWithProducts(new Guid(id));
            model.CurrentQuote.User = GetQuoteUser(model.CurrentQuote);
            QuoteNotifier.SendQuoteStateNotification(model.CurrentQuote.pk, this.DefaultImgPath, false, notifyEshopgloziksoft: false);
