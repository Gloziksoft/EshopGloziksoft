...
namespace eshopgloziksoft.lib.Controllers.Ecommerce
{
    ...
    public class CustomerRegisterController : _BaseController
    {
        ...
        public ActionResult SubmitRegistration(CustomerRegisterModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                EshopgloziksoftMember newMember = EshopgloziksoftMemberModel.CreateCopyFrom(model);

                EshopgloziksoftMemberRepository memberRep = new EshopgloziksoftMemberRepository();
                EshopgloziksoftMember member = null;
                ...
                if (member != null)
                {
                    EshopgloziksoftCustomerRepository customerRep = new EshopgloziksoftCustomerRepository();
                    EshopgloziksoftCustomer customer = null;
                    if (customerRep.Save(EshopgloziksoftCustomer.CreateCopyFrom(model, member)))
                    {
                        //new EshopgloziksoftNewsletterRepository().SetForEmail(model.Email, model.AgreePersonalDataNewsletter);
                ...
                return this.RedirectToEshopgloziksoftUmbracoPage(ConfigurationUtil.EcommerceRegistrationOkFormId);
            }
        ...
        public ActionResult SubmitLostPassword(LostPasswordModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                EshopgloziksoftMember member = repository.GetMemberByEmail(model.Email);
                if (member == null)
                {
