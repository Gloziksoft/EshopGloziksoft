...
namespace eshopgloziksoft.lib.Controllers.Ecommerce
{
    ...
    public class CustomerDataController : _BaseController
    {
        ...
        public ActionResult SaveMyProfile(CustomerModel model)
        {
            ...
            if (new EshopgloziksoftCustomerRepository().Save(CustomerModel.CreateCopyFrom(model, new CustomerDropDowns())))
            {
        ...
        public ActionResult SaveMyPassword(ChangePasswordModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                EshopgloziksoftMemberModel memberModel = EshopgloziksoftMemberModel.CreateCopyFrom(repository.Get(CustomerModel.GetCurrentMemberId()));
                memberModel.Password = model.NewPassword;
                memberModel.PasswordRepeat = model.NewPasswordRepeat;
                MembershipCreateStatus status = repository.SavePassword(EshopgloziksoftMemberModel.CreateCopyFrom(memberModel));
