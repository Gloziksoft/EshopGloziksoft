...
{
    public class EshopgloziksoftProductRepository : _BaseRepository
    {
        static readonly object _productOrderLock = new object();

        public Page<EshopgloziksoftProduct> GetPage(long page, long itemsPerPage, string sortBy = null, string sortDir = null, EshopgloziksoftProductFilter filter = null)
        {
            ...
            return GetPage<EshopgloziksoftProduct>(page, itemsPerPage, sql);
        }

        public Page<EshopgloziksoftProduct> GetPageForCategory(long page, long itemsPerPage, Guid categoryKey)
        {
        ...
        public Page<EshopgloziksoftProduct> GetPageForNotInCategory(Guid categoryKey, long page, long itemsPerPage, string sortBy = "ProductOrder", string sortDir = "ASC", EshopgloziksoftProduct2CategoryFilter filter = null)
        {
            ...
            sql.Append(string.Format("ORDER BY {0}.{1} {2}", EshopgloziksoftProduct.DbTableName, sortBy, sortDir));

            return GetPage<EshopgloziksoftProduct>(page, itemsPerPage, sql);
        }

        public Page<EshopgloziksoftProduct> GetPageForQuote(long page, long itemsPerPage, Guid quoteKey)
        {
        ...
        public Page<EshopgloziksoftProduct> GetPageForSearch(long page, long itemsPerPage, string searchText)
        {
        ...
        public EshopgloziksoftProduct Get(Guid key)
        {
            var sql = GetBaseQuery().Where(GetBaseWhereClause(), new { Key = key });

            return Fetch<EshopgloziksoftProduct>(sql).FirstOrDefault();
        }
        public EshopgloziksoftProduct GetForProductCode(string code)
        {
            var sql = GetBaseQuery().Where(GetCodeWhereClause(), new { Code = code });

            return Fetch<EshopgloziksoftProduct>(sql).FirstOrDefault();
        }
        public EshopgloziksoftProduct GetForProductUrl(string url)
        {
        ...
        {
            var sql = new Sql(string.Format("SELECT DISTINCT(producerKey) FROM {0}", EshopgloziksoftProduct.DbTableName));
        ...
        public bool Save(EshopgloziksoftProduct dataRec)
        {
            if (IsNew(dataRec))
            {
        ...
        bool Insert(EshopgloziksoftProduct dataRec)
        {
        ...
        bool Update(EshopgloziksoftProduct dataRec)
        {
            return UpdateInstance(dataRec);
        }

        public bool Delete(EshopgloziksoftProduct dataRec)
        ...
        public bool SetNewProductOrder(EshopgloziksoftProduct dataRec)
        {
            var sql = new Sql();
            sql.Append(string.Format("UPDATE {0} SET {1} = (SELECT MAX({1})+1 FROM {0}) WHERE {2}=@Key",
                EshopgloziksoftProduct.DbTableName, "ProductOrder", "pk"),
                new { Key = dataRec.pk });
        ...
        public bool ReorderProducts(int orderFrom, int orderOffset)
        {
            ...
            sql.Append(string.Format("UPDATE {0} SET {1}={1}+@OrderOffset WHERE {1}>=@OrderFrom",
                EshopgloziksoftProduct.DbTableName, "ProductOrder"),
        ...
        public int GetProductOrder(Guid productKey)
        {
            ...
            sql.Append(
                ...
                EshopgloziksoftProduct.DbTableName),
                new { ProductKey = productKey });
        ...
        Sql GetBaseQuery()
        {
            return new Sql(string.Format("SELECT * FROM {0}", EshopgloziksoftProduct.DbTableName));
        }

        string GetBaseWhereClause()
        {
            return string.Format("{0}.pk = @Key", EshopgloziksoftProduct.DbTableName);
        }
        string GetUrlWhereClause()
        {
            return string.Format("{0}.productUrl = @Url", EshopgloziksoftProduct.DbTableName);
        }
        string GetCodeWhereClause()
        {
            return string.Format("{0}.productCode = @Code", EshopgloziksoftProduct.DbTableName);
        }
        string GetOnlyVisibleWhereClause()
        {
            return string.Format("{0}.productIsVisible = 1", EshopgloziksoftProduct.DbTableName);
        }
        string GetRelatedProductWhereClause()
        {
            return string.Format("{0}.pk IN (SELECT {1}.pkProductRelated FROM {1} WHERE {1}.pkProductMain = @KeyProductMain)", EshopgloziksoftProduct.DbTableName, EshopgloziksoftProductRelation.DbTableName);
        }
        string GetProductInWhereClause(List<string> productKeyList)
        {
            return string.Format("{0}.pk IN ({1})", EshopgloziksoftProduct.DbTableName, GetKeysForInClause(productKeyList));
        }
        string GetProductCategoryInWhereClause(List<string> productCategoryKeyList)
        {
            return string.Format("{0}.pk IN (SELECT {1}.pkProduct FROM {1} WHERE {1}.pkCategory IN ({2}))", EshopgloziksoftProduct.DbTableName, EshopgloziksoftProduct2Category.DbTableName, GetKeysForInClause(productCategoryKeyList));
        }
        string GetProducerInWhereClause(List<string> producerKeyList)
        {
            return string.Format("{0}.producerKey IN ({1})", EshopgloziksoftProduct.DbTableName, GetKeysForInClause(producerKeyList));
        }
        string GetProductAttributeInWhereClause(List<string> productAttributeKeyList)
        {
            return string.Format("{0}.pk IN (SELECT {1}.pkProduct FROM {1} WHERE {1}.pkAttribute IN ({2}))", EshopgloziksoftProduct.DbTableName, EshopgloziksoftProduct2Attribute.DbTableName, GetKeysForInClause(productAttributeKeyList));
        }
        string GetProductAttributeWhereClause(string productAttribute)
        {
            return string.Format("{0}.pk IN (SELECT {1}.pkProduct FROM {1} WHERE {1}.pkAttribute='{2}')", EshopgloziksoftProduct.DbTableName, EshopgloziksoftProduct2Attribute.DbTableName, productAttribute);
        }
        string GetSearchTextWhereClause(string searchText)
        {
            ...
            foreach (string searchItem in items)
            {
                ...
                str.Append(string.Format("({0}.productCode LIKE '%{1}%' collate Latin1_general_CI_AI OR {0}.productName LIKE '%{1}%' collate Latin1_general_CI_AI OR {0}.productDescription LIKE '%{1}%' collate Latin1_general_CI_AI OR {0}.productImg LIKE '%{1}%' collate Latin1_general_CI_AI)", EshopgloziksoftProduct.DbTableName, searchItem));
            }
        ...
        {
            return string.Format("{0}.pk IN (SELECT {1}.pkProduct FROM {1} WHERE {1}.pkCustomer = @FavoriteProductsCustomerKey)", EshopgloziksoftProduct.DbTableName, Product2CustomerFavorite.DbTableName);
        }
        ...
        {
            var sql = new Sql();
            sql.Append(string.Format("SELECT {0}.* FROM {0}, {1} WHERE {0}.pk = {1}.pkProduct AND {1}.pkCategory = @Key ORDER BY {1}.productOrder", EshopgloziksoftProduct.DbTableName, "epProduct2Category"), new { Key = categoryKey });
        ...
        {
            var sql = new Sql();
            sql.Append(string.Format("SELECT {0}.* FROM {0}", EshopgloziksoftProduct.DbTableName));

            return sql;
        }
        string GetNotInCategoryWhereClause()
        {
            return string.Format("{0}.pk NOT IN (SELECT {1}.pkProduct FROM {1} WHERE {1}.pkCategory = @Key)", EshopgloziksoftProduct.DbTableName, EshopgloziksoftProduct2Category.DbTableName);
        }

        Sql GetProductsInQuoteQuery(Guid quoteKey)
        {
            var sql = new Sql();
            sql.Append(string.Format("SELECT {0}.*, {1}.itemOrder FROM {0}, {1} WHERE {0}.pk = {1}.pkProduct AND {1}.pkQuote = @Key ORDER BY {1}.itemOrder", EshopgloziksoftProduct.DbTableName, "epProduct2Quote"), new { Key = quoteKey });
        ...
        {
            var sql = new Sql();
            sql.Append(string.Format("SELECT {0}.* FROM {0} WHERE {0}.ProductCode LIKE '%{1}%' collate Latin1_general_CI_AI OR {0}.ProductName LIKE '%{1}%' collate Latin1_general_CI_AI OR {0}.ProductDescription LIKE '%{1}%' collate Latin1_general_CI_AI OR {0}.ProductImg LIKE '%{1}%' collate Latin1_general_CI_AI ORDER BY {0}.productOrder", EshopgloziksoftProduct.DbTableName, searchText));

            return sql;
        }
    }

    [TableName(EshopgloziksoftProduct.DbTableName)]
    [PrimaryKey("pk", AutoIncrement = false)]
    public class EshopgloziksoftProduct : _BaseRepositoryRec
    {
    ...
    public class EshopgloziksoftProductFilter
    {
